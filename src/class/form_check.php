...
class form{
    ...
    private function trueCount(array $elementList):int{
        ...
        foreach($elementList as $element){
            ...
            // checkbox are not counted if they are not checked
            $methodUsed = (format::normalize($this->method)=="post") ? $_POST : $_GET;

            $checkboxNameList = array();
    ...
    private function trueEnabledFieldList():array{
        ...
        for($i=0; $i<count($elementList); $i++){
            $checkboxNameList = array();
            if(array_key_exists("tag", $elementList[$i])){
                if($elementList[$i]["tag"]=="input"){
                    if(array_key_exists("attributList", $elementList[$i])){
                        if(array_key_exists("type", $elementList[$i]["attributList"])){
                            if($elementList[$i]["attributList"]["type"]=="checkbox"){
                                $checkboxNameList[] = $elementList[$i]["attributList"]["name"]; // i add the name of the checkbox in the array
                            }
                        }
                    }
                }
